...
contract("Election", function(accounts){
    ...
    it("allows a voter to vote", function(){
        return Election.deployed().then(function(instance){
            ...
            assert.equal(receipt.logs[0].args._candidateId.toNumber(), candidateId, "the voted candidated is correct");
            ...
            assert.equal(voteCount,1,"increments the candidates 2 vote count");
        });
    });
    it("throws exception for invalid candidates", function(){
        return Election.deployed().then(function (instance) { 
            ...
            assert.equal(voteCount, 1, "candidate 0 did not receive any votes");
            return electionInstance.candidates(3)
    ...
    it("throws exception for double voting", function(){
        return Election.deployed().then(function (instance) {
            electionInstance = instance;
            candidateId = 3;
            electionInstance.vote(candidateId, {from: accounts[4]});
            return electionInstance.candidates(candidateId);
        }).then(function(candidate){
            var voteCount = candidate[2];
            assert.equal(voteCount, 1, "accepts first vote");
            return electionInstance.vote(candidateId, {from: accounts[4]});
        }).then(assert.fail).catch(function(error){
            assert(error.message.indexOf('revert') >= 0, "error message must contain revert");
            return electionInstance.candidates(1);
        }).then(function (candidate1) {
